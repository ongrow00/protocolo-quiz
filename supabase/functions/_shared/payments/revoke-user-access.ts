...
import type { NormalizedBuyer, ProvisionUserResult } from './types.ts';
...
async function findProfileByEmail(
	...
	if (error) {
		console.error('[payments] profile lookup failed:', error.message);
...
export async function revokeProductAccess(
	...
	if (updateError) {
		console.error('[payments] profile revoke failed:', updateError.message);
