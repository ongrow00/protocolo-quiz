import type { LastlinkWebhookData } from './types.ts';
...
/** Maps Products[].Name to profile access flags. */
export function resolveProductAccess(data: LastlinkWebhookData | undefined): ProductAccessFlags {
	const names = (data?.Products ?? []).map((p) => normalizeProductName(p.Name ?? ''));
	return {
